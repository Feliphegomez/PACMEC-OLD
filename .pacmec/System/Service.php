...
class Service extends \PACMEC\System\BaseRecords
{
  ...
  public function set_all($obj)
  {
    ...
    if($this->isValid()){
      ...
      $this->link_view = __url_S("/%services_view%/{$this->id}/".urlencode($this->name));
      $this->link_edit   = __url_S("/%admin_services_slug%?service_id={$this->id}");
      $this->link_remove = __url_S("/%admin_services_slug%?remove_service={$this->id}");
      ...
      $this->gallery = [];
      foreach ($GLOBALS['PACMEC']['DB']->FetchAllObject("SELECT * FROM `{$GLOBALS['PACMEC']['DB']->getTableName('services_pictures')}` WHERE `service` IN (?) ORDER BY `ordering`", [$this->id]) as $picture)
      {
  ...
  public static function table_list_html(array $items) : String
  {
    ...
    $table->addHeaderRow([
      \PHPStrap\Util\Html::tag('a', \PHPStrap\Util\Html::tag('i', '', ['fa fa-plus']), ['btn btn-sm btn-outline-success btn-hover-success'], [
        'href'=>__url_s("/%admin_services_slug%?create_item=true")
      ])
      , __a('slug')
      , __a('name')
      , __a('common_names')

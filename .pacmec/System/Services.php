...
class Services extends \PACMEC\System\ModeloBase {
  ...
  public static function byId($id)
  {
    ...
    if($item->id > 0) return new Service((object) ['id'=>$item->id]);
  ...
  public static function allNews($limit=null) : array
  {
    ...
    if(\infosite('with_gallery')==true) {
      $sql_gallery = "SELECT GROUP_CONCAT(PP.`service`) as ids
        FROM `{$GLOBALS['PACMEC']['DB']->getTableName('services_pictures')}` PP
        INNER JOIN `{$GLOBALS['PACMEC']['DB']->getTableName('services')}` P
    ...
    foreach($GLOBALS['PACMEC']['DB']->FetchAllObject($sql, []) as $item){
      $r[] = new Service((object) ['id' => $item->id]);
    }
    return $r;
  }
